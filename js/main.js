...
(function () {
  ...
  SECTIONS.forEach(function (s) {
    ...
    li.innerHTML = '<a href="#' + s.id + '" data-target="' + s.id + '">' + esc(s.label) + "</a>";
  ...
  (function renderHero() {
    var links = [];
    if (profile.email) {
      links.push('<a href="mailto:' + esc(profile.email) + '">' + icon("mail") + "<span>Email</span></a>");
    }
    if (profile.linkedin) {
      links.push('<a href="' + esc(profile.linkedin) + '" target="_blank" rel="noopener">' + icon("linkedin") + "<span>LinkedIn</span></a>");
    }
    if (profile.github) {
      links.push('<a href="' + esc(profile.github) + '" target="_blank" rel="noopener">' + icon("github") + "<span>GitHub</span></a>");
    }
    ...
    document.getElementById("hero-content").innerHTML =
      ...
      '<img class="hero-photo" src="' + esc(profile.image || "assets/profile-placeholder.svg") + '" ' +
      'alt="Portrait of ' + esc(name) + '" width="255" height="255"></div>' +
      '<p class="hero-hello" id="hero-hello" aria-label="' + esc(HELLO) + '">' +
      '<span id="hero-hello-text" aria-hidden="true"></span></p>' +
      '<h1 class="hero-name" id="hero-name" aria-label="' + esc(name) + '">' +
      '<span class="name-highlight" id="hero-name-text" aria-hidden="true"></span></h1>' +
      '<p class="hero-tagline reveal hero-late" style="--d:0ms">' + esc(profile.tagline || "") + "</p>" +
      '<p class="hero-intro reveal hero-late" style="--d:110ms">' + esc(profile.intro || "") + "</p>" +
      '<div class="hero-links reveal hero-late" style="--d:220ms">' + links.join("") + "</div>";
  ...
  (function renderEducation() {
    ...
    target.innerHTML = list
      .map(function (e, i) {
        ...
        return (
          '<div class="edu-entry reveal" style="--d:' + i * 90 + 'ms">' +
          '<div class="edu-icon">' + icon("gradcap") + "</div>" +
          "<div>" +
          "<h3>" + esc(e.school) +
          (hasGoose
            ? '<span class="goose-badge" title="Quality assured by the campus geese">' + GOOSE + "</span>"
            : "") +
          "</h3>" +
    ...
    target.innerHTML =
      ...
      list
        .map(function (e, i) {
          ...
          var logoHtml = e.logo
            ? '<img class="company-logo" src="' + esc(e.logo) + '" alt="' + esc(e.company) + ' logo" loading="lazy">'
            : '<span class="company-logo company-logo-fallback" aria-hidden="true">' +
              esc((e.company || "?").replace(/^REPLACE ME — /, "").charAt(0).toUpperCase()) +
              "</span>";
          return (
            '<div class="timeline-item reveal-x" style="--d:' + i * 110 + 'ms">' +
            ...
            "<h3>" + esc(e.title) + "</h3>" +
            '<span class="t-company">' + esc(e.company) + "</span>" +
            '<span class="t-period">' + esc(e.period) + "</span>" +
            "</header></div>" +
            '<ul class="t-points">' +
            (e.points || []).map(function (pt) { return "<li>" + esc(pt) + "</li>"; }).join("") +
            "</ul></article></div>"
          );
        })
        .join("");
    ...
    target.innerHTML = list
      .map(function (p, i) {
        ...
        var imgHtml = imgs.length
          ? '<div class="project-images cols-' + imgs.length + '">' +
            imgs.map(function (src) {
              return '<img src="' + esc(src) + '" alt="' + esc(p.name) + ' screenshot" loading="lazy">';
            }).join("") +
            "</div>"
          : "";
        var actions = "";
        if (p.github) {
          actions +=
            '<a class="btn btn-ghost" href="' + esc(p.github) + '" target="_blank" rel="noopener">' +
            icon("github") + "<span>Code</span></a>";
        }
        if (p.demo) {
          actions +=
            '<a class="btn btn-primary" href="' + esc(p.demo) + '" target="_blank" rel="noopener">' +
            icon("external") + "<span>Live Demo</span></a>";
        }
        return (
          '<div class="reveal" style="--d:' + (i % 2) * 90 + 'ms">' +
          '<article class="card project-card">' +
          imgHtml +
          "<h3>" + esc(p.name) + "</h3>" +
          "<p>" + esc(p.description) + "</p>" +
          ((p.tech || []).length
            ? '<ul class="tags">' + p.tech.map(function (t) { return "<li>" + esc(t) + "</li>"; }).join("") + "</ul>"
            : "") +
    ...
    target.innerHTML = list
      .map(function (p, i) {
        var title = p.link
          ...
          : esc(p.title);
        return (
          '<div class="reveal" style="--d:' + i * 80 + 'ms">' +
          ...
          (p.link
            ? '<a class="pub-link" href="' + esc(p.link) + '" target="_blank" rel="noopener">' +
              icon("external") + "<span>View paper</span></a>"
            : "") +
          "</article></div>"
        );
      })
    ...
    if (photos.length) {
      html +=
        ...
        photos
          .map(function (ph, i) {
            return (
              '<div class="reveal" style="--d:' + i * 100 + 'ms">' +
              '<figure class="polaroid">' +
              '<img src="' + esc(ph.src) + '" alt="' + esc(ph.caption || "a photo of my interests") + '" loading="lazy">' +
  ...
  (function renderFooter() {
    var links = [];
    if (profile.email) {
      links.push('<a href="mailto:' + esc(profile.email) + '" aria-label="Email">' + icon("mail") + "</a>");
    }
    if (profile.linkedin) {
      links.push('<a href="' + esc(profile.linkedin) + '" target="_blank" rel="noopener" aria-label="LinkedIn">' + icon("linkedin") + "</a>");
    }
    if (profile.github) {
      links.push('<a href="' + esc(profile.github) + '" target="_blank" rel="noopener" aria-label="GitHub">' + icon("github") + "</a>");
    }
    document.getElementById("footer-links").innerHTML = links.join("");

    document.getElementById("footer-note").innerHTML =
      "© " + new Date().getFullYear() + " " + esc(profile.name || "") +
  ...
  if ("IntersectionObserver" in window) {
    ...
    visibleSections.forEach(function (sec) { sectionObserver.observe(sec); });
  }
  ...
  mascot.addEventListener("click", function () {
    ...
    honkTimer = window.setTimeout(function () { bubble.classList.remove("show"); }, 1700);
  });
})();
